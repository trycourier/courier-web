...
- Reports live at `api/*.api.md`; per-package config is each package's `api-extractor.json`. Scripts: `scripts/generate-api-docs.sh` (update) and `scripts/build-packages-ci.sh` (build + check, used by CI's `build-packages.yml`).
