...
# Courier Banner Web Components

Embed customizable banner notifications in your web app using Web Components. Courier Banner provides the `<courier-banner>` element that works with any JavaScript framework or vanilla JS. Banners are synced with the feed of Courier Inbox messages.
...
```bash
npm install @trycourier/courier-ui-banner
```
...
If you've already set up authentication for [Courier Inbox](https://github.com/trycourier/courier-web/tree/main/%40trycourier/courier-ui-inbox), you can skip this step; banners and inbox share an authentication mechanism.
...
- **Click Handling** — `onBannerItemClick()` and `onBannerItemActionClick()` for banner and action button clicks
- **Theming** — full light/dark theme support with `setLightTheme()` and `setDarkTheme()`
- **Custom Elements** — replace banner content (`setBannerItemContent`) or entire banner items (`setBannerItem`)
- **Datastore** — `CourierBannerDatastore.shared.addMessage()` and `removeMessage()` for programmatic control

## Documentation

Full documentation: **[courier.com/docs/sdk-libraries/courier-ui-banner-web](https://www.courier.com/docs/sdk-libraries/courier-ui-banner-web/)**
...
- [Sample app (Vanilla JS)](https://github.com/trycourier/courier-samples/tree/main/web/vanilla/banner)
